...
U_one   = 2.3
J_one   = 0.35
Ntot    = latt.L*4*4   # 4 Ru per unit cell, 4 electrons per Ru.

numberkana = latt.L*4*3 # 4 Ru per unit cell, 3 kanamori per Ru
site_i = []; site_j = []
for i in range(latt.L*4):
    site_i.append( 0+i*3 ); site_i.append( 0+i*3 ); site_i.append( 1+i*3 )
    site_j.append( 1+i*3 ); site_j.append( 2+i*3 ); site_j.append( 2+i*3 )

U1  = [U_one-2*J_one]*latt.L*12
U2  = [U_one-3*J_one]*latt.L*12
J   = [J_one]*latt.L*12
...
#Total state is 4*3*latt.L, additional 2 for soc
Tmatrix = np.zeros(( 24 * latt.L, 24 * latt.L), dtype='complex', order='F')
    ...
    for dx in jumpx:
        ...
        for dy in jumpy:
            ...
            for dz in jumpz:
                ...
                j = latt.index( coor_j )
                Tmatrix[ 12*i:12*(i+1), 12*j:12*(j+1) ] += Hopping[(dx, dy, dz)]

Tmatrix[ 12*latt.L:24*latt.L, 12*latt.L:24*latt.L ] = Tmatrix[ 0:12*latt.L, 0:12*latt.L ]
...
f.write( '{:16d} \n'.format(numberkana) )
f.write( '{:>16} \n'.format("charge") )
for i in range( 24*latt.L ):
    for j in range( 24*latt.L ):
...
initOrderParamType        =  "afmxy"       # zero, rhfRandom, uhfRandom, ghfRandom, afmz, afmx, afmxy
...
if initOrderParamType is not None:
    subprocess.call( "python {0}/initialOrderParameterCa2RuO4.py {1}".format(pythonFileDir, initOrderParamType),shell=True)
